...
Get-ADTWinGetPackage [[-Query] <String>] [-MatchOption <String>] [-Command <String>] [-Count <UInt32>]
 [-Id <String>] [-Moniker <String>] [-Name <String>] [-Source <String>] [-Tag <String>] [-Scope <String>] [<CommonParameters>]
```
...
```powershell
Get-ADTWinGetPackage -Scope machine
```
...
```powershell
Get-ADTWinGetPackage -Id "Microsoft.PowerShell" -Scope machine
```

This example shows how to get a specific package by its identifier from the machine scope.
...
Specify the install scope to search for packages. Valid values are 'user' and 'machine'.
