...
function Get-ADTWinGetExitCodeTable
{
    ...
    if ($Dynamic)
    {
        # Open object for returning to the caller.
        $obj = [ordered]@{}

        # Process the source code for its exit codes.
        (Invoke-RestMethod -UseBasicParsing -Uri 'https://raw.githubusercontent.com/microsoft/winget-cli/master/src/AppInstallerSharedLib/Public/AppInstallerErrors.h').Split("`n") | & {
            process
            {
                if ($_ -match '^.+_ERROR_.+0x\w{8}')
                {
        ...
        # Throw if we somehow have a zero count in our object.
        if (!$obj.Count)
        {
            $naerParams = @{
                Exception = [System.IO.InvalidDataException]::new("Failed to retrieve exit codes from WinGet's source code. The source code may have changed and can no longer be parsed.")
                Category = [System.Management.Automation.ErrorCategory]::InvalidResult
                ErrorId = 'WinGetExitCodeParsingError'
                TargetObject = $obj
                RecommendedAction = "Please review the source code at [https://raw.githubusercontent.com/microsoft/winget-cli/master/src/AppInstallerSharedLib/Public/AppInstallerErrors.h] for changes, then try again."
            }
