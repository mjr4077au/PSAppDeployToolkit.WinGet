BeforeAll {
    ...
    #-------------------------------------------------------------------------
    $ModuleName = 'PSAppDeployToolkit.WinGet'
...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module PSAppDeployToolkit.WinGet.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain PSAppDeployToolkit.WinGet.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "PSAppDeployToolkit.WinGet.psm1"
        } #validPSM1
